...
Bot.main = function() {
	try {
		...
		haxe_Log.trace("Possibly Fatal Error. " + Std.string(e) + haxe_CallStack.toString(haxe_CallStack.exceptionStack()),{ fileName : "Source/Bot.hx", lineNumber : 23, className : "Bot", methodName : "main"});
	}
};
Bot.__init = function() {
	try {
		...
		haxe_Log.trace("Error in evalhandler. " + e.details(),{ fileName : "Source/Bot.hx", lineNumber : 31, className : "Bot", methodName : "__init"});
	}
	try {
		lib_Logger.setTrace();
	} catch( _g ) {
		var e = haxe_Exception.caught(_g);
		haxe_Log.trace("Error in logger. " + e.details(),{ fileName : "Source/Bot.hx", lineNumber : 36, className : "Bot", methodName : "__init"});
	}
	try {
		Bot.start();
	} catch( _g ) {
		var e = haxe_Exception.caught(_g);
		haxe_Log.trace("Error in start. " + e.details(),{ fileName : "Source/Bot.hx", lineNumber : 41, className : "Bot", methodName : "__init"});
	}
};
Bot.tock = function() {
};
Bot.start = function() {
	...
	Bot.bot.on("ready",function(e) {
		Bot.bot.channels.fetch("704748213655175300",false);
	});
	Bot.bot.on("message",CommandHandler.handle);
	Bot.bot.login(Bot.getToken()).then(function(e) {
		haxe_Log.trace("Ready!",{ fileName : "Source/Bot.hx", lineNumber : 52, className : "Bot", methodName : "start"});
	});
};
