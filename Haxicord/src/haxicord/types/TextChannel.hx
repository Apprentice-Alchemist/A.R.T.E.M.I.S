...
class TextChannel extends GuildChannel implements MessageChannel {
    ...
    @:dox(hide)
    public function new(_chan:haxicord.types.structs.GuildChannel.TextChannel, _client:DiscordClient) {
        client = _client;
    ...
    @:dox(hide)
    public function _update(_chan:haxicord.types.structs.GuildChannel.TextChannel) {
        if(_chan.name != null)
            name = _chan.name;
