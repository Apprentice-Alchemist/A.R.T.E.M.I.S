    ...
    static function _makeRequest(url,method="GET",_callback:Null<Dynamic->Map<String,String>->Void>=null,_d:Dynamic=null,_headers:Map<String,String>=null,isJson=true){
        ...
        call.onError = function(no){
           ...
           // trace(call.responseData);
            #if neko
                response=neko.Lib.stringReference(result.getBytes());
            #else
                response=result.getBytes().toString();
            #end
            hasError=true;
            ...
            if(errReg.match(no)){
                _cb({"status":status,"error":"HTTP error "+status,"data":isJson?parseJson(status,response,true):response},m);
            }else{
                _cb({"status":status,"error":no,"data":isJson?parseJson(status,response,true):response},m);
            }
        }
        var post=false;
        if(["POST","PUT","PATCH"].indexOf(method)>-1&&_data!=null){
        ...
        call.customRequest(post,result,null,method);
        if( !hasError )
		#if neko
			call.onData(response=neko.Lib.stringReference(result.getBytes()));
		#else
			call.onData(response=result.getBytes().toString());
        #end
#end
        }catch(er:Dynamic){
