...
class DiscordClient {
	...
	function receiveEvent(m) {
		...
		switch (m.t) {
			...
			case "READY":
				...
				// trace(re.guilds);
				unavailableGuilds = re.guilds.filter(function(g) {
					return g.unavailable;
				}).length; // assume all guilds unavail
			...
			case "GUILD_CREATE":
				if (ready) {
					...
					unavailableGuilds = dataCache.getAllGuilds().filter(function(g) {
						return g.unavailable;
					}).length; // assume all guilds unavail
	...
	public dynamic function _onReady() {
		onReady();
	}
...
private class HeartbeatThread {
	...
	public function beat() {
		...
		#else
		while (going) {
			while (Sys.time() - cl.lastbeat < delay) {
				Sys.sleep(0.5);
			}
			cl.lastbeat = Sys.time();
			ws.sendJson(WSPrepareData.Heartbeat(seq));
